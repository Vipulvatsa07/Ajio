  ...
  const getData = async () => {
    ...
    else
    {
      try {
        ...
        let res = await axios.get(
          `https://mockserver-rm4.onrender.com/data?category=women-clothing`
        );
        setData(res.data);
        // setFooter(true);
      } catch (error) {
    ...
    >
      {windowWidth > 1000 ? <SidebarLarge all={"women-clothing"} firstpart={"saree"} second={"shirts-tops-tunics"} third={"kurta-kurti"} fourth={"kurta-suit-sets"}/> : null}
        ...
        <Box
          ...
          bg={"whitesmoke"}
        >
          {windowWidth < 1000 ? <Sidebar all={"women-clothing"} firstpart={"saree"} second={"shirts-tops-tunics"} third={"kurta-kurti"} fourth={"kurta-suit-sets"}/> : null}
          <Box>
        ...
        >
          {data.map((elem) => {
            return (
              <Box key={elem.id}>
                <Link  to={`/women/${elem.id}`}><Image src={elem.image} alt="image not found" /></Link>
                <Link  to={`/women/${elem.id}`}><Text textColor={"#B19975"} as="b">
                  {elem.brand}
                </Text></Link>
